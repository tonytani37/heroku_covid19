...
def data_set(summary_json):
    ...
    data_n = [str(i+1).zfill(2)+':'+row['name_ja']
              for i, row in enumerate(summary_json['prefectures'])]  # 都道府県名
#     data_n = [str(i+1).zfill(2)+':'+row['name'] for i,row in enumerate(summary_json['prefectures'])] #都道府県名
    data_l = [row['dailyConfirmedCount']
              for row in summary_json['prefectures']]  # 感染者数
    data_d = [row['dailyDeceasedCount']
              for row in summary_json['prefectures']]  # 死亡者数
    ...
    # dataframeはcopyしないとワーニングが出る
    df_a = df[['date', 'confirmed', 'confirmedAvg7d', "criticalCumulative"]].copy()
    ...
    df_a = df_a.rename(
        columns={'date': '日付', 'confirmed': "感染者数",
                 'confirmedAvg7d': '感染者数移動平均', "criticalCumulative": '重症者数'}
    )
    ...
    ax2 = ax1.twinx()
    ax1.bar(df_a['日付'].tail(DAYS), df_a['感染者数'].tail(
        DAYS), label='日次感染者数', color='lightgray')
    ax1.plot(df_a['日付'].tail(DAYS), df_a['感染者数移動平均'].tail(
        DAYS), label='感染者数移動平均', color='red', linewidth=2)
    ax2.plot(df_a['日付'].tail(DAYS), df_a['検査数移動平均'].tail(
        DAYS), label='検査数移動平均', color='green', linewidth=2)
    ...
    # df_ps['重症者']['2021-02-21'] += 500
    # df_show['重症者数'][389] += 500
...
# 東京都
    ...
    df_d = pd.DataFrame([row['diagnosed_date']]
                        for row in summary_json_t['data'])
    df_r = pd.DataFrame([row['weekly_gain_ratio'], row['count']]
                        for row in summary_json_t['data'])
    ...
    ax1.hlines(y=1, xmin=min(df['日付']),
               xmax=max(df['日付']), color='red')
...
if __name__ == "__main__":
    ...
    fig, fig3, figt, df_show, update, df_l, df_d, df_p, df_ps, df, df_total, df_a = data_set(
        summary_json)
    ...
    # st.sidebar.write(' ')
    st.sidebar.write(
        'このサイトでは「日本国内の新型コロナウイルス (COVID-19) 感染状況追跡」(https://covid19japan.com)で作成されたデータを利用してます')
    # st.sidebar.write(' ')
    ...
    if option == '全国感染者情報':
        ...
        def kjitai_area(erea_list):
            list = ["埼玉県", "千葉県", "東京都", "神奈川県", "岐阜県", "愛知県", "京都府", "大阪府", "兵庫県", "福岡県"
                    ]
        ...
        dateList_date = [datetime(row.year, row.month, row.day)
                         for row in df_date[-DAYS:]]
        ...
        radio = st.radio('対象データ', (('移動平均', '実数', '実数(積み上げ棒グラフ）'
                                    )))
        ...
        selected_erea = st.multiselect(
            '都道府県を選択してください', erea_list, default=kjitai_ereas)
        df = df_l[(df_l['都道府県'].isin(selected_erea))]
        if len(df) > 0:
            df_lt = df.T
            df_lta = df_lt.drop(['都道府県', '合計'], axis=0)
            df_lta['date'] = dateList_date
            df_lta = df_lta[(df_lta['date'] >= dateF) &
                            (df_lta['date'] <= dateT)]

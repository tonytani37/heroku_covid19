...
data_l = pd.DataFrame([row['dailyConfirmedCount'] for row in summary_json['prefectures']]) #感染者数
data_d = pd.DataFrame([row['dailyDeceasedCount'] for row in summary_json['prefectures']]) #死亡者数
...
df_l = pd.DataFrame(data_l.tail(days),columns=df_date.tail(days))
df_l['name_ja'] = df_n.tail(days)
# df_l = df_l.set_index('name_ja')
df_sum = df_l.sum(axis=1)
df_l['sum'] = df_sum
df_l = df_l.fillna(0)

df_d = pd.DataFrame(data_d.tail(days),columns=df_date.tail(days))
df_d['name_ja'] = df_n.tail(days)
